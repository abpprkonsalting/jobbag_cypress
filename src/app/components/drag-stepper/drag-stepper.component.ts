...
import { ChangeDetectorRef, Component, OnInit, Injectable, Input,
          Output, EventEmitter, OnDestroy, ViewChild, ElementRef, AfterViewChecked } from '@angular/core';
...
export class DragStepperComponent  extends MatStepper implements OnInit, OnDestroy, AfterViewChecked {
  ...
  private _returnDecision = false;
  cdr: ChangeDetectorRef;


  constructor(dir: Directionality, changeDetectorRef: ChangeDetectorRef,
              public breakpointObserver: BreakpointObserver, protected messageSubscriber: DragStepperMessagesHandle<Partial<any>>) {
  ...
  checkDraggable() {
    if (this.contentInner.nativeElement.scrollHeight === this.contentInner.nativeElement.clientHeight) {
          this.dragDisabled = false; } else { this.dragDisabled = true;
          }
  ...
  initExternalMessagesInput() {
    this.stepperSubscriptionIndex = this.messageSubscriber.subscribe(message => {
        console.log(message);
  ...
  ngOnDestroy() {
    if (this.stepperSubscriptionIndex !== undefined) { this.messageSubscriber.unsubscribe(this.stepperSubscriptionIndex); }
  }
