...
export class Step4Component implements OnInit, OnDestroy {
  ...
  onMouseUpAdd(object) {
    if ((object.$event.x <= this.clickPosition.x + 10) && (object.$event.x >= this.clickPosition.x - 10) &&
        (object.$event.y <= this.clickPosition.y + 10) && (object.$event.y >= this.clickPosition.y - 10)) {
          ...
          const selected = this.availableExperiences.filter(exp => exp.profession.id === object.experience.profession.id)[0];
  ...
  addExperiences() {
    const selectedExperiences = this.availableExperiences.filter(experience => experience.profession.selected === true);
