...
export class Employee {
  ...
  getDto(){
    ...
    if ((this.id == undefined || this.id == null) && (this.resume == '' && this.experiences.length == 0 && this.locations.length == 0 )) {
      return null;
    }

    let exp = this.experiences.map(experience => experience.getDto());

    // For sending the locations to the backend it's not necessary to send the parent / childs structure, just the enumeration of all of theme.
    let workingLocs = this.flatenlocations(this.locations);
